...
@section('contents')
    <div class="card">
        <div class="card-header">
            <h5 class="text-uppercase">Buat absen untuk kelas <b>{{ $jadwal->ruangan->name }}</b></h5>
        </div>
        <div class="card-body col-md-8 col-sm">
            <form action="{{ route('absensi.store') }}" method="post">
                @csrf
                <input type="hidden" value="{{ encrypt($jadwal->id) }}" name="jadwal">
                <input type="hidden" name="kelas" value="{{ $jadwal->ruangan->id }}">
                <x-input attr="matkul" label="Matakuliah" readonly value="{{ $jadwal->mata_pelajaran->name }}" />
